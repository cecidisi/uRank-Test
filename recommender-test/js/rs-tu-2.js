/*******************************************************************************
*
*   avg normaliaztion on tag and user scores, avg normalization on final score
*
********************************************************************************/
...
window.RS_TU_2 = (function(){
    ...
    RS_TU_2.prototype = {
        ...
        getRecommendations: function(args) {
            ...
            var sumUserScores = 0, sumTagScores = 0;
            ...
            _.keys(_this.itemTagMatrix).forEach(function(doc){
                ...
                if(!_this.userItemMatrix[p.user] || !_this.userItemMatrix[p.user][doc]){
                    ...
                    if(p.options.beta < 1) {
                        ...
                        sumUserScores += userBasedScore;
                    }

                    if(isNaN(userBasedScore) || isNaN(tagBasedScore)) {
                        console.log('********** TU_2 has NAN');
                        console.log('tag score is NAN = ' + isNaN(tagBasedScore));
                        console.log('user score is NAN = ' + isNaN(userBasedScore));
                    }
...
//            console.log('sumTagScores = ' + sumTagScores);
//            console.log('sumUserScores = ' + sumUserScores);
            recs.forEach(function(r){
                var tagBasedScore = parseFloat(r.misc.tagBasedScore/sumTagScores) || 0.0,
                    userBasedScore = parseFloat(r.misc.userBasedScore/sumUserScores) || 0.0;
                r.score = tagBasedScore * p.options.beta + userBasedScore * (1 - p.options.beta);
                
//                console.log('TU norm = ' + r.score);
//                console.log('TU NO norm = ' + (r.misc.tagBasedScore * p.options.beta + r.misc.userBasedScore * (1-p.options.beta)));
            });
